...
AGENTIC_WORK = HERE.parent

DEFAULT_DB = AGENTIC_WORK / "application" / "data" / "tasks.db"
...
DEFAULT_APP_CONFIG = AGENTIC_WORK / "application" / "config.json"
...
def resolve_tasks_db_for_user(user_id: str) -> Path:
    ...
    working = AGENTIC_WORK / "application" / "data" / "users" / f"{segment}.db"
...
def session_storage_dir() -> Path:
    load_env()
    default = AGENTIC_WORK / "application" / ".session_storage"
...
def load_app_config() -> dict[str, Any]:
    """Load agentic-work application/config.json only (no other repos)."""
    path = Path(os.getenv("APP_CONFIG_PATH", str(DEFAULT_APP_CONFIG))).expanduser()
    if not path.is_absolute():
        path = (AGENTIC_WORK / path).resolve()
    if not path.is_file():
        return {}
