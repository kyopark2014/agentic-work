...
# Configuration
project_name = "agentic-work" # at least 3 characters
region = "us-west-2"
...
# HMAC for my-schedule skill + Lambda → ECS (AgentCore may read this; not session-signing-key).
...
def get_or_create_cloudfront_signing_material(*, rotate: bool = False) -> Dict[str, str]:
    ...
    if not key_group_id:
        ...
        # Ensure the key group still references our public key.
        try:
...
def _ecs_agent_runtime_resource_arns() -> List[str]:
    ...
    projectName. Scan application/config.json and all runtime_agent/*/config.json
    for naming conventions plus any explicit agent_runtime_arn.
    """
    root = os.path.dirname(os.path.abspath(__file__))
    configs: List[Dict] = [
        _read_local_json_config(os.path.join(root, "application", "config.json"))
    ]
    runtime_agent_root = os.path.join(root, "runtime_agent")
    if os.path.isdir(runtime_agent_root):
        for entry in sorted(os.listdir(runtime_agent_root)):
            config_path = os.path.join(runtime_agent_root, entry, "config.json")
            if os.path.isfile(config_path):
                configs.append(_read_local_json_config(config_path))
...
def _cloudfront_s3_cache_behavior(
    ...
    if key_group_id:
        # Require CloudFront signed cookies/URLs for S3-backed prefixes.
...
def get_or_create_cloudfront_response_headers_policy() -> str:
    ...
    passing through ``Server: uvicorn``. CSP stays in app middleware (Google Sign-In).
    """
    ...
    try:
        ...
        # Race or eventual consistency: re-list once by name.
...
def deploy_ecs_service(
    ...
    environment = [
        ...
        {
            "name": "SCHEDULE_JOBS_TABLE",
            "value": str(public_app_config.get("schedule_jobs_table") or SCHEDULE_JOBS_TABLE_NAME),
        },
        {
            "name": "SCHEDULE_GROUP_NAME",
            "value": str(public_app_config.get("schedule_group_name") or SCHEDULE_GROUP_NAME),
        },
        {
            "name": "SCHEDULE_LAMBDA_ARN",
            "value": str(public_app_config.get("schedule_lambda_arn") or ""),
        },
        {
            "name": "SCHEDULER_ROLE_ARN",
            "value": str(public_app_config.get("scheduler_role_arn") or ""),
        },
    ]
...
def main():
    ...
    if args.schedule_only:
        ...
        if not sharing:
            raise SystemExit(
                ...
                "(run a full install first, or set sharing_url)."
            )
    ...
    try:
        ...
        # 7.5 my-schedule: DynamoDB + Lambda + EventBridge Scheduler
        schedule_info = deploy_schedule_infrastructure(
        ...
        # (e.g. agentic_work-*), not only root project_name naming.
        ecs_roles = create_ecs_roles()
